...
trait OrderOper
{
    ...
    public static function pre($params, $calc_type = 'pre')
    {
        ...
        foreach ($new_goods_list as $key => $buyinfo) {
            ...
            if ($detail['type'] == 'normal' && $user_address) {
                $current_dispatch_amount = Dispatch::getDispatch($dispatch_type, $detail, $user_address, $buyinfo['goods_num']);
                // $current_dispatch_amount = 0;       // 暂时设置为 0 
    ...
    public static function coupons($params, $goods_amount = 0)
    {
        ...
        foreach ($coupons as $key => $coupon) {
            ...
            // 商品可用 并且 商品金额满足
            if (!$is_break && $coupon->enough <= $goods_amount) {
                $new_coupons[] = $coupon;
            }
        }
    ...
    public static function createOrder($params, $is_job = false)
    {
        $user = User::info();
        ...
        $order = Db::transaction(function () use (
            ...
            foreach ($new_goods_list as $key => $buyinfo) {
                $detail = $buyinfo['detail'];
                $current_suk_price = $detail['current_sku_price'];
                ...
                $orderItem->activity_id = $current_suk_price['activity_id'] ?? 0;     // 商品当前的活动类型
                $orderItem->activity_type = $current_suk_price['activity_type'] ?? null;     // 商品当前的活动类型
                // 当前商品规格对应的 活动下对应商品规格的 id
                $orderItem->item_goods_sku_price_id = isset($current_suk_price['item_goods_sku_price']) ?
                    $current_suk_price['item_goods_sku_price']['id'] : 0;
                $orderItem->goods_sku_text = $current_suk_price['goods_sku_text'];
                $orderItem->goods_title = $detail->title;
                $orderItem->goods_image = empty($current_suk_price['image']) ? $detail->image : $current_suk_price['image'];
                ...
                $orderItem->dispatch_fee = $dispatch_amount;
                $orderItem->dispatch_type = $detail->dispatch_type;
                $orderItem->dispatch_id = $detail->dispatch_ids;
    ...
    public static function operConfirm($params)
    {
        ...
        $order = Db::transaction(function () use ($order, $item, $user) {
            ...
            OrderAction::operAdd($order, $item, $user, 'user', '用户确认收货');
