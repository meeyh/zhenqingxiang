...
class Goods extends Model
{
    ...
    public static function operActivitySkuPrice($detail, $sku_price) {
        ...
        if ($activity) {
            $detail['price'] = min(array_column($sku_price, 'price'));

            if ($activity['type'] == 'groupon') {
                $detail['groupon_price'] = min(array_column($sku_price, 'groupon_price'));
            }
    ...
    public function getCouponsAttr($value, $data)
    {
        ...
        $coupons = Coupons::where('find_in_set(:id,goods_ids)', ['id' => $goods_id])
            ->where([
                'deletetime' => null,
//                'starttime' => ['elt', time()],
//                'endtime' => ['egt', time()]
    ...
    private static function getGoodsListOrder($orderStr)
    {
        ...
        if (isset($priceOrder) && $priceOrder === 1) {
            $order = "convert(`price`, DECIMAL) asc";
        }elseif (isset($priceOrder) && $priceOrder === 2) {
            $order = "convert(`price`, DECIMAL) desc";
        }
